...
function Home() {
    ...
    return (
        <>
            <section className="relative bg-cover bg-center bg-no-repeat h-[30vh] md:h-[80vh] flex items-center justify-center text-center"
                style={{ backgroundImage: "url('https://images.unsplash.com/photo-1504674900247-0877df9cc836?fm=jpg&q=60&w=3000&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D')" }}
            >
                {/* Overlay */}
                <div className="absolute inset-0 bg-black/50"></div>

                {/* Content */}
                <div className="relative z-10 max-w-xs md:max-w-2xl text-white px-4">
                    <h1 className="text-2xl md:text-6xl font-robotoslabsemibold mb-4">
                        Find the Best Restaurants Near You
                    </h1>
                    <p className="text-sm font-robotoslab md:text-xl mb-6">
                        Explore menus, reviews, and cuisines powered by Tripadvisor API
                    </p>
                </div>
            </section>
            <div className="mx-5 lg:mx-10 lg:p-4 mt-3">
                    ...
                    {list.map((r, i) => (
                        <div key={i} className="border-2 border-[#FF9B00] rounded-xl py-2 px-3 hover:-translate-y-3">
                            ...
                            <div className="flex justify-between mt-1">
                                ...
                                <p className={`flex gap-1 items-center text-xs lg:text-base text-center font-robotoslab ${r.currentOpenStatusCategory?.toLowerCase().includes("open") ? "text-green-600" : "text-red-600"}`}>
                                    <svg
                                        xmlns="http://www.w3.org/2000/svg"
                                        viewBox="0 0 24 24"
                                        fill="none"
                            ...
                            <div className="grid grid-cols-2 gap-2 mt-2">
                                {r.establishmentTypeAndCuisineTags.slice(0, 2).map((tag, i) => (
                                    <p
                                        key={i}
                                        className="bg-[#FF9B00] text-[6px] lg:text-xs text-center font-robotoslab py-1 px-2 rounded-xl"
                                    >
